import { PostItem as PostItemType } from '@/app/lib/posts/types'
  ...
  return (
    <Link href={`/posts/${post.slug}`} className="h-full">
      <motion.div
        ...
        className="outline outline-[1px] outline-primary px-5 py-6 rounded-lg bg-white hover:outline-[3px] overflow-hidden md:min-h-full"
      >
        <div className="flex flex-col items-start gap-3">
          <div className="flex items-center gap-2 justify-between w-full">
            ...
            {newPost && <Badge className="bg-orange">NEW</Badge>}
          </div>
          ...
          <p className="line-clamp-3">{post.meta_default_description}</p>
        </div>
      </motion.div>
    </Link>
  )
