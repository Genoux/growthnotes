...
function useQueryPosts(params: FetchPostsParams = {}, enabled: boolean = true): UseQueryResult<Post[], Error> {
...
export function useSinglePost(slug?: string) {
  ...
  const post = posts && slug ? posts.find(p => p.slug === slug) : posts?.[0] || null;
  return { data: post, ...rest };
}

export function usePosts(limit?: number) {
  const { data: posts, ...rest } = useQueryPosts({}, true);
  const filteredPosts = posts && limit && limit > 0 ? posts.slice(0, limit) : posts;
